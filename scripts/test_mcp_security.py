    ...
    def test_valid_token(self):
        ...
        if status == 200:
            if data and "result" in data:
                ...
                self.print_info(f"Available tools: {', '.join([t.get('name', '?') for t in tools[:3]])}")
            else:
...
Examples:
  ...
  python test_mcp_security.py http://localhost:9105/mcp
    ...
    parser.add_argument(
        "url",
        help="MCP endpoint URL (e.g., http://localhost:8000/mcp)"
    )
    
    parser.add_argument(
        "--bearer", "-b",
        help="Bearer token for authenticated tests",
        metavar="TOKEN"
    )
